...
- **Animation semantics:** Each process card has a stable identity and exists in exactly one visible scheduler location: the CPU or one ready queue. At every adjacent dispatch, return, rotation, preemption, demotion, boost, arrival, completion, and reverse step, typed simulator phases—not event-text parsing—drive the visible state. A stationary arrow points toward the active destination while the process card moves, and a short live cue explains it. Multi-event boundaries play serially; simultaneous moves of the same type share one numbered guide. Followers temporarily retain their old queue slots until a departing card clears them, preventing overlaps without inventing scheduler events. Direct timeline jumps are inspection-only, controls lock synchronously, and resize/scroll cancels safely to the authoritative state. Animation never changes scheduling results, and reduced-motion users receive the same final state without movement or a playback lock.
...
- `npm run test:e2e` runs 40 Chromium checks across 1920×1080 and 1366×768. It traverses all 21 boundaries of the lecture workload for every policy; checks exact CPU, lifecycle, and queue state; verifies forward/reverse phase order, compound MLFQ boundaries, protected boosts, same-time arrivals, queue landing and non-overlap geometry, rapid-input locks, final autoplay, reduced motion, cleanup, and large-scenario accessibility without page-level vertical scrolling.
