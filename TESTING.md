...
- **Same-time event ordering:** At boundary `t`, completion is recorded first. On ordinary boundaries, arrivals enter before an expired or yielding process is enqueued. On MLFQ boost boundaries, expiry/yield is enqueued first, waiting processes are boosted, and then arrivals enter. Ordinary preemption and dispatch follow. Thus an allotment that expires exactly at a boost boundary is demoted first and then boosted; RR arrivals enter before the expired process.
...
- `npm run test:e2e` runs 52 Chromium checks across 1920×1080 and 1366×768. It traverses all 21 boundaries of the lecture workload for every policy; checks exact CPU, lifecycle, and queue state; verifies forward/reverse phase order, FCFS/SJF/STCF/RR transfer corridors, STCF preemption, compound and mixed-level MLFQ boosts, protected boosts, same-time arrivals, queue landing, frame-sampled traffic separation, rapid-input locks, final autoplay, reduced motion, cleanup, persistent movement review with forward/reverse steps and scenario reset, and large-scenario accessibility without page-level vertical scrolling.
...
Explanation regression tests check the displayed decision against queue state, remaining times, stable ties, quantum usage, protected boosts, idle gaps, and completion. Invalid scheduling settings render an actionable validation message instead of throwing during component rendering.
