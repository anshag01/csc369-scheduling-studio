...
- `npm run test:e2e` runs 46 Chromium checks across 1920×1080 and 1366×768. It traverses all 21 boundaries of the lecture workload for every policy; checks exact CPU, lifecycle, and queue state; verifies forward/reverse phase order, default Round Robin rotation corridors, compound and mixed-level MLFQ boosts, protected boosts, same-time arrivals, queue landing, frame-sampled traffic separation, rapid-input locks, final autoplay, reduced motion, cleanup, and large-scenario accessibility without page-level vertical scrolling.
