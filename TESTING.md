...
- **Animation semantics:** Each process card has a stable identity and exists in exactly one visible scheduler location: the CPU or one ready queue. At every adjacent dispatch, return, rotation, preemption, demotion, boost, arrival, completion, and reverse step, the process card is the only moving object. A stationary arrow points toward its destination while that movement plays, and a short live cue explains it. Multi-event boundaries are staged one event type at a time, and simultaneous moves of the same type are grouped behind one numbered arrow; hover or keyboard focus reveals its full process/destination detail. Direct timeline jumps are inspection-only so the UI never invents a route across skipped states. Step controls remain locked until the explanation finishes. Animation never changes scheduling state, and reduced-motion users receive the same final state without movement.
