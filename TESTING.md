...
- `npm run test:e2e` runs Chromium checks with desktop and compact browser profiles, plus explicit viewport coverage from 320×568 through 1920×1080. It traverses all 21 boundaries of the lecture workload for every policy; checks exact CPU, lifecycle, and queue state; verifies forward/reverse phase order, FCFS/SJF/STCF/RR transfer corridors, STCF preemption, compound and mixed-level MLFQ boosts, protected boosts, same-time arrivals, queue landing, frame-sampled traffic separation, rapid-input locks, final autoplay, reduced motion, cleanup, and large-scenario accessibility through pagination without page or panel scrolling. Layout checks cover mobile section tabs, both mobile metrics views, and 50 processes with six-character IDs. Set `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` to use an existing local Chrome installation.
